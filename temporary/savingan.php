...
$radius = isset($_GET['radius']) ? floatval($_GET['radius']) : 7;

if ($user_lat && $user_lon) {
    $sql = "SELECT nama_komunitas, jenis_aktivitas, jadwal_bermain, biaya, alamat_bermain, deskripsi_komunitas, contact, latitude, longitude, 
            (6371 * acos(cos(radians(?)) * cos(radians(latitude)) * cos(radians(longitude) - radians(?)) + sin(radians(?)) * sin(radians(latitude)))) AS distance
            FROM communities HAVING distance <= ? ORDER BY distance";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("dddi", $user_lat, $user_lon, $user_lat, $radius);
    $stmt->execute();
    $result = $stmt->get_result();
} else {
    $sql = "SELECT nama_komunitas, jenis_aktivitas, jadwal_bermain, biaya, alamat_bermain, deskripsi_komunitas, contact FROM communities";
...
<div class="community-info-box">
    <div id="userPreferenceInfo">
        <?php if ($user_lat && $user_lon): ?>
            <span>Menampilkan komunitas terdekat berdasarkan lokasi Anda.</span>
        <?php else: ?>
...
function getUserLocation() {
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(function(position) {
            window.location.href = `mau-main-bareng.php?lat=${position.coords.latitude}&lon=${position.coords.longitude}`;
        });
    } else {
